...
const profissionalService = require('../service/profissional_service')
...
router.post('/api/profissional', async function (req, res) {
    ...
    if (await profissionalService.existeNome(profissional)) {
        res.status(404).send("Profissional já existe!")
    } else {
        const newProfissional = await profissionalService.saveProfissional(profissional)
        if (newProfissional.status === 1){
            res.status(404).send("ja existe profissional com esse usuario")
        }else{
